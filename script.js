...
const btnJugar = document.createElement("button");//creamos boton cerrar
btnJugar.textContent = "Jugar";
btnJugar.setAttribute("id", "jugar");    
modal.append(btnJugar);
btnJugar.addEventListener("click", () => btnJugar);//cerramos el modalDeInicio

const btnCargar = document.createElement("button");//creamos boton para cargar una partida
btnCargar.textContent = "Cargar Partida";
btnCargar.setAttribute("id", "cargar");    
modal.append(btnCargar);
btnCargar.addEventListener("click", () => btnCargar);//cerramos el modal de Inicio

btnCargar.onclick =function(){      
    
    
    //funcionalidades para cargar partidas guardadas  


    modal.style.visibility= "hidden";
}
btnJugar.onclick =function(){     
    modal.removeChild(btnCargar); 
    modal.removeChild(btnJugar);                   
     
    const labelNombre = document.createElement("label");//creamos una etiqueta para el nombre cerrar
    labelNombre.textContent = "Ingrese su Nombre";
    labelNombre.setAttribute("id", "labelNombre");    
    modal.append(labelNombre);

    const inputNombre = document.createElement("input");//creamos una caja de texto para el nombre cerrar    
...
const iniciarJuego= () => { //aquí comienza el juego
    ...
    const darMensaje = (mensaje) => { //funcion que crea y muestra mensajes
        ...
        modal.append(elementoMensaje);
        setTimeout(() => modal.removeChild(elementoMensaje), 3000);
